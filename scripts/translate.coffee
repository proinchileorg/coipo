...
module.exports = (robot) ->
  ...
  pattern = new RegExp('traduce?' +
                       "(?: de (#{language_choices}))?" +
                       "(?: (?:in)?a (#{language_choices}))?" +
                       '(.*)', 'i')
          ...
          if body.length > 4 and body[0] == '['
            ...
            if parsed
              if msg.match[2] is undefined
                msg.send "#{term} en #{language} es #{parsed}"
              else
                msg.send "#{term} en #{language} se traduce como #{parsed} en #{languages[target]}"
          else
